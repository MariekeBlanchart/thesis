...
##gatebuilding    
    ...
    gatespolygon = constructsegments(coords[numrunways*5:])

    ## if constructintervals didn't produce annything -> return 1000000
    if gatespolygon and gatespolygon.is_valid:
...
def constructsegments(coords):
    ...
    for i in range(0, maxsegments):
        heightsegment = 90*(int(coords[2 + i]) % 9 - 4)
        points.append((startx + 90*i, starty + heightsegment))
        points.append((startx + 90*i + 90, starty + heightsegment))
        
    startx = startx + 90*maxsegments
    starty = starty + 90*(int(coords[2 + maxsegments - 1]) % 9 - 4)
    
    for i in range(0, maxsegments):
        heightsegment = 90*(int(coords[2 + i]) % 9 - 4)
        points.append((startx + heightsegment, starty - 90*i))
        points.append((startx + heightsegment, starty - 90*i - 90))
        
    startx = startx + 90*(int(coords[2 + maxsegments - 1]) % 9 - 4)
    starty = starty - 90*maxsegments
...
## Optimalisation
es = cma.CMAEvolutionStrategy(numrunways * [airport.centroid.coords[0][0], airport.centroid.coords[0][1], minrunwaylength, minrunwaywidth*100, 0] + 7*[0], 1000)
optimise(airport, numrunways, fitness, constructsegments, es, printpreviousresult)

win = graphics.GraphWin('L-systeem', 600, 600) # give title and dimensions
win.setCoords(-1000, -1000, 1000, 1000)
    
#lsystem = constructsegments([2,2,5,7,5,3,6])
# lsystem = constructsegments([1,5,3,0,5545455,2,8])
# print([x % 9 - 4 for x in [1,5,3,0,5545455,2,8]])
# print(lsystem)
#     
# try:
